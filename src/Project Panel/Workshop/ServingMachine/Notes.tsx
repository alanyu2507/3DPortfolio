export default function Notes() {
  ...
  return (
    <div className="hudProjectPanel__contentBody">
      ...
      <img src={`${imageBase}Wheel.webp`} alt="Custom contoured wheel CAD render" className="hexapodArchitecture__diagram"/>
      <img src={`${imageBase}Axis.webp`} alt="Wheel axis component render" className="hexapodArchitecture__diagram"/>
      <p className="hudProjectPanel__contentText">
        After 3D printing the wheel and attaching it to a motor, I tested its grip by launching a ball against the floor.
      </p>
      <video
        src={`${imageBase}single.webm`}
        autoPlay
        loop
        muted
        playsInline
        aria-label="Single-wheel launch grip test"
      ...
      <img src={`${imageBase}Final.webp`} alt="Final serving machine build" />
    </div>
  );
}
